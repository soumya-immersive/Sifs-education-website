        ...
        <div className="text-right space-y-1">
          {internship.created_at && (
            <span className="block text-[10px] font-medium text-gray-400 uppercase tracking-wider">
              Published: {new Date(internship.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
            </span>
          )}
          {internship.updated_at ? (
            <span className="block font-bold text-[11px] text-blue-600 uppercase tracking-tight">
              Updated: {new Date(internship.updated_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
            </span>
          ) : (
