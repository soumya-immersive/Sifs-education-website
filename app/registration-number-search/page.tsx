        ...
        const fetchPageData = async () => {
            try {
                const response = await fetch(
                    `${process.env.NEXT_PUBLIC_API_BASE_URL}/EducationAndInternship/Website/registration/number-search`
                );
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                setTimeout(() => {
                    router.push(data.redirect?.url || "/show-upload-document");
                }, 1500);
            } else {
        ...
        <section className="relative bg-white pt-16 pb-24 px-4 overflow-hidden min-h-screen">
            ...
            <div className="max-w-7xl mx-auto relative z-10 text-center mb-12">
                ...
                <h1 className="text-4xl md:text-6xl font-bold text-gray-900 mb-6 tracking-tight">
                    {pageData?.title || "Search Registration"}
                </h1>

                {/* Description */}
                <p className="max-w-2xl mx-auto text-gray-600 text-lg md:text-xl mb-10 leading-relaxed">
                    {pageData?.description || "Enter your registration number to find your course details"}
                </p>
                ...
                <div className="max-w-2xl mx-auto">
                    <div className="flex flex-col md:flex-row gap-3 bg-white p-2 rounded-2xl md:rounded-full shadow-2xl border border-gray-100">
                        <div className="flex-1 relative flex items-center">
                            ...
                            <input
                                type="text"
                                placeholder="Enter Registration Number (e.g. 1212273102)"
                                value={regNumber}
