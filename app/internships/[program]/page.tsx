...
export default function InternshipProgramPage({ params: paramsPromise }: { params: Promise<{ program: string }> }) {
  ...
  const {
    ...
    saveData,
    isLoaded
  } = useDynamicPageData("internships");
  ...
  // 1. Filter courses (internships) for this program
  ...
  const handleSave = async () => {
    ...
    setTimeout(() => {
      if (success) {
        setEditMode(false);
        toast.success("✅ Internships saved successfully");
      } else {
        toast.error("❌ Failed to save internships");
      }
      setIsSaving(false);
    }, 800);
  };
  ...
  const handleAddCourse = () => {
    ...
    const newItem = {
      ...
      programSlug: program,
      slug: `new-internship-${Date.now()}`,
      title: "New Internship Title",
      courseCode: "INT-XXX",
      overview: "New internship overview description here...",
      description: "Full description comes here.",
      descriptionParts: ["First paragraph of the new internship description."],
      rating: 5.0,
      reviewsCount: 0,
      ...
      sidebarData: {
        ...
        instructors: [
          { name: "Expert Trainer", role: "Internship Lead", image: "/course/ins1.png" }
        ]
      },
      highlights: [
        "Hands-on forensic project",
        "Expert mentorship",
        "Real-world case exposure"
      ],
      accordionItems: [
        { title: "Week 1: Introduction", content: "Onboarding and basics." }
      ],
      paymentDetails: {
        paypal: "",
        bankName: "ICICI BANK",
  ...
  const handleDeleteCourse = (id: number) => {
    ...
    if (confirm(`Are you sure you want to delete "${itemToDelete.title}"?`)) {
      ...
      updateSection("courses", updatedCourses);
      toast.success("🗑️ Internship removed successfully");
    }
  };
