...
const LocationItem = ({
  ...
  return (
    <div className="relative group/location">
      ...
      >
        <span
          className={`text-[14px] font-bold ${isOpen ? "text-blue-600" : "text-gray-700"} transition-colors uppercase tracking-wide`}
        >
          {item.title}
        </span>
        <div
          className={`p-1.5 rounded-lg ${isOpen ? "bg-blue-600 text-white" : "bg-gray-50 text-gray-400"} group-hover:bg-blue-600 group-hover:text-white transition-all duration-300`}
        >
          <ChevronDown
...
export default function ContactPage() {
  ...
  const [helpSettings, setHelpSettings] = useState<any>(null);
  const [loadingHelpSettings, setLoadingHelpSettings] = useState(true);

  useEffect(() => {
    const fetchPageData = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/EducationAndInternship/Website/front/contact`,
        );
        const result = await response.json();
        if (
          !result.success &&
          result.message === "Contact section is disabled"
        ) {
    ...
    const fetchLocations = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/EducationAndInternship/Website/contact-locations`,
        );
        const result = await response.json();
        if (result.success) {
    ...
    const fetchHelpSettings = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/EducationAndInternship/Website/front/help-settings`,
        );
        const result = await response.json();
        if (result.success) {
          setHelpSettings(result.data);
        }
      } catch (error) {
        console.error("Error fetching help settings:", error);
      } finally {
        setLoadingHelpSettings(false);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/EducationAndInternship/Website/front/sendmail`,
        {
          method: "POST",
          headers: {
      ...
      if (result.success) {
        setStatus({
          type: "success",
          message: result.message || "Contact query sent successfully!",
        });
        setForm({
          first: "",
          email: "",
        ...
        {!isPageEnabled ? (
          ...
          >
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-4">
              {disabledMessage || "Section Disabled"}
            </h2>
          </motion.div>
        ) : (
          <>
                ...
                <div className="p-8 pt-6">
                  <div className="flex items-start gap-4 p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50 mb-8">
                    ...
                    <div className="text-sm">
                      <p className="font-bold text-gray-900 uppercase tracking-wider text-[11px] mb-1">
                        {helpSettings?.office_title}
                      </p>
                      <p className="text-gray-700 leading-relaxed font-medium hover:text-blue-600 transition-colors">
                        <Link
                          href={helpSettings?.map_link}
                          target="_blank"
                          rel="noopener noreferrer"
                        >
                          {helpSettings?.office_address}
                        </Link>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-5">
                    {[
                      {
                        img: "/contact/2.png",
                        title: helpSettings?.education_title || "Education",
                        phone: helpSettings?.education_numbers,
                        email: helpSettings?.education_emails,
                      },
                      {
                        img: "/contact/3.png",
                        title: helpSettings?.training_title || "Training",
                        phone: helpSettings?.training_numbers,
                        email: helpSettings?.training_emails,
                      },
                      {
                        img: "/contact/4.png",
                        title: helpSettings?.internship_title || "Internship",
                        phone: helpSettings?.internship_numbers,
                        email: helpSettings?.internship_emails,
                      },
                      {
                        img: "/contact/5.png",
                        title: helpSettings?.workshop_title || "Workshop",
                        phone: helpSettings?.workshop_numbers,
                        email: helpSettings?.workshop_emails,
                      },
                    ].map((item, index) => (
                      <div key={index} className="flex gap-4 items-start group">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-[13px] text-gray-600 mt-1 flex flex-wrap items-center gap-x-2">
                            {(Array.isArray(item.phone) ? item.phone : []).map((p: any, i: number, arr: any[]) => (
                              p && (
                          ...
                          <div className="text-[13px] text-gray-600 mt-1 flex flex-wrap items-center gap-x-2">
                            {(Array.isArray(item.email) ? item.email : []).map((e: any, i: number, arr: any[]) => (
                              e && (
                                <span key={i} className="flex items-center gap-2">
                                  <Link
                                    href={`mailto:${e.trim()}`}
